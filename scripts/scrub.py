                ...
                if is_jsonl:
                    ...
                    try:
                        ...
                        scrubbed_lines.append(
                            json.dumps(scrubbed_event, ensure_ascii=False)
                        )
    ...
    def check_file(self, file_path: Path) -> List[str]:
        ...
        try:
            ...
            for pattern, _ in SECRET_PATTERNS:
                ...
                if matches:
                    violations.append(
                        f"Found {len(matches)} secret(s) matching pattern: {pattern}"
                    )
            ...
            non_team_emails = [
                e
                for e in emails
                if e.lower() not in {e.lower() for e in self.team_emails}
            ]
            if non_team_emails:
                violations.append(
                    f"Found {len(non_team_emails)} non-team email(s): {non_team_emails[:3]}"
                )
            ...
            for pattern, _ in PATH_PATTERNS:
                ...
                if matches:
                    violations.append(
                        f"Found {len(matches)} absolute path(s): {matches[:3]}"
                    )
...
def scrub_single_file(input_path: str, output_path: str, team_emails: Set[str]) -> int:
    ...
    if scrubber.scrub_file(input_file, output_file):
        ...
        print("✗ Scrubbing failed", file=sys.stderr)
        return 1
...
def check_directory(check_dir: str, team_emails: Set[str]) -> int:
    ...
    files = [
        f for f in check_path.rglob("*") if f.is_file() and not f.name.startswith(".")
    ]
...
def main():
    ...
    mode_group.add_argument("input", nargs="?", help="Input JSONL file")
    mode_group.add_argument("--dir", help="Input directory (scrub all JSONL files)")
    ...
    # Output
    parser.add_argument("output", nargs="?", help="Output JSONL file or directory")
    parser.add_argument(
        "--output", dest="output_dir", help="Output directory (for --dir mode)"
    )

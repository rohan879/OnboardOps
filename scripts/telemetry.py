...
class TelemetryCapture:
    ...
    def __init__(
        ...
        logger.info(f"Telemetry capture initialized")
        ...
        try:
            ...
            if not session_id:
                ...
                if event.get('type') == 'SessionStart':
                    session_id = event.get('id') or f"session-{datetime.utcnow().timestamp()}"
                else:
            ...
            if self.event_count % 10 == 0:
                logger.info(f"Captured {self.event_count} events across {len(self.sessions)} active sessions")

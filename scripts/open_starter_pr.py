    ...
    for cmd in test_commands:
        try:
            ...
            if result.returncode == 0:
                ...
                test_count = (
                    output.count("PASSED") + output.count("passed") + output.count("✓")
                )
                if test_count > 0:
...
def open_pr(
    repo_full_name: str, branch_name: str, title: str, body: str, token: str
) -> Optional[str]:
...
def check_bob_shell_available() -> bool:
    ...
    try:
        result = subprocess.run(["bob", "--version"], capture_output=True, timeout=5)
...
def generate_diff_with_bob(
    repo_path: Path, task_type: int, candidate_info: Dict
    ...
    try:
        ...
        try:
            ...
            if not all(field in response for field in required_fields):
                print("  ⚠️  Bob response missing required fields")
                return None
...
def main():
    ...
    parser.add_argument(
        "--candidate", type=int, default=1, help="Candidate number (1, 2, or 3)"
    )
    parser.add_argument(
        "--onboardee", type=str, default="demo", help="Onboardee name for branch naming"
    )
    parser.add_argument(
        "--repo-path",
        type=Path,
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Simulate without actually creating PR"
    )
    parser.add_argument(
        "--use-bob",
        action="store_true",
    ...
    try:
        ...
        if not args.dry_run:
            ...
            emit_event(
                "StarterPRTestStart",
                {"repo_path": str(repo_path), "candidate": args.candidate},
            )
            ...
            if test_success:
                ...
                emit_event(
                    "StarterPRTestPass",
                    {"repo_path": str(repo_path), "output_length": len(test_output)},
                )
            else:
        ...
        agents_md_link = (
            f"[View personalized AGENTS.md](../blob/{branch_name}/AGENTS.md)"
        )
...
"""
        ...
        if not args.dry_run and pr_url:
            ...
            print("  2. Verify tests pass in CI")
            print("  3. Merge when ready")
        else:
            print("  1. Run without --dry-run to actually create PR")
            print("  2. Ensure demo repo is cloned and accessible")
            print("  3. Verify GitHub token has write access")
            if args.use_bob:
                print("  4. Ensure Bob Shell is installed and authenticated")
        ...
        if checkpoint and not args.dry_run:
            ...
            try:
                ...
                print("  Git status should be clean now")

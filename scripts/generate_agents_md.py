...
def merge_user_sections(new_content: str, user_sections: Dict[str, str]) -> str:
    ...
    for section_name, user_content in user_sections.items():
        ...
        if section_start != -1:
            ...
            lines = lines[:section_start] + user_content.split('\n') + lines[section_end:]
            merged_content = '\n'.join(lines)
...
def load_certification_results(session_file: Optional[Path] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        if total > 0:
            print(f"   Found {total} certification questions ({certification['passed']} passed)")
...
def generate_agents_md_with_bob(
    ...
    context_parts.append("")
    context_parts.append("Using the agents-md-recipe skill, generate a complete AGENTS.md file with ALL FIVE sections:")
    context_parts.append("1. Repo Cartography Summary - assembled from all four cartography stages above")
    ...
    context_parts.append("4. Open Questions - include failed/partial certification questions")
    context_parts.append("5. Recommended Next Reading - Bob-curated from cartography output")
    context_parts.append("")
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            return generate_fallback_agents_md(repo_name, structure, cartography, certification)
    ...
    if cartography['stage3']:
        ...
        if hotspots:
            ...
            for hs in hotspots[:5]:
                hotspots_section += f"- `{hs.get('file', '')}` - {hs.get('churn', 0)} changes\n"
        else:
    ...
    if cartography['stage4']:
        ...
        if conventions:
            for conv in conventions[:5]:
                conventions_section += f"- **{conv.get('name', '')}**: {conv.get('description', '')}\n"
        else:
            conventions_section += "No conventions detected.\n"
    else:
        conventions_section += "Convention analysis pending (requires Stage 4 cartography).\n"
    conventions_section += "\n"
    
    # Section 4: Open Questions
    open_questions = "## 4. Open Questions\n\n"
    if certification['questions']:
        failed_questions = [q for q in certification['questions'] if q['result'] in ['fail', 'partial']]
        if failed_questions:
            open_questions += "Questions from certification that need deeper exploration:\n\n"
...
def main():
    ...
    if args.no_bob:
        ...
        content = generate_fallback_agents_md(repo_name, structure, cartography, certification)
    else:
